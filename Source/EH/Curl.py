...
def M_Ez_Curl_Ex(Ez,dy):#curl of E pointing in z dir 
	...
	for nx in range(Nx):
		...
		CEx[nx,Ny-1]=0
		#(0-Ez[nx,Ny-1])/dy#boundary condition case
	return CEx
...
def M_Ez_Curl_Hz(Hx, Hy, dx, dy):
	...
	CHz[0,0]=0
	#(Hy[0,0]-0)/dx - (Hx[0,0]-0)/dy #sepcial case 
	for nx in range (1,Nx):
		CHz[nx,0]=0
		#(Hy[nx,0]-Hy[nx-1,0])/dx - (Hx[nx,0]-0)/dy#firs colomn condition
	for ny in range (1,Ny):
		CHz[0,ny]=0
		#(Hy[0,ny]-0)/dx - (Hx[0,ny]-Hx[0,ny-1])/dy#first row special case
